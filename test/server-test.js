...
describe("Persistant Server", function() {
  ...
  after(function(done) {
    fs.unlink("./data/garrettdeick.json", function(err) {
      done();
    });
  });

  describe("GET request", function() {
    it("should respond to GET at /api/users/username for non-existing user with 404 status code", function(done) {
      chai.request("http://localhost:8080")
        .get("/api/users/erincolt")
    ...
    it("should respond to GET at /api/users/username for existing user with JSON file", function(done) {
      chai.request("http://localhost:8080")
        .get("/api/users/colincolt")
        .end(function(err, res) {
          expect(err).to.eql(null);
  ...
  describe("POST request", function() {
    ...
    it("should create a file (username.json) in data directory on POST where no resource, send 201 (created)", function(done) {
      chai.request("http://localhost:8080")
        .post("/api/users/marccolt")
        .send(marcData)
        .end(function(err, res) {
          expect(err).to.eql(null);
          expect(res).to.have.status(201);
          fs.readFile("./data/marccolt.json", function(err, data) {
            expect(data.toString()).to.eql(JSON.stringify(marcData));
            done();
          });
        });
    });
  ...
  describe("PUT request", function() {
    it("should respond with 201 (created) on PUT where resource does not exist, create file (username.json)", function(done) {
      chai.request("http://localhost:8080")
        .put("/api/users/garrettdeick")
        .send({"username":"garrettdeick"})
        .end(function(err, res) {
          expect(err).to.eql(null);
          expect(res).to.have.status(201);
          fs.readFile("./data/garrettdeick.json", function(err, data) {
            expect(data.toString()).to.eql(JSON.stringify({"username":"garrettdeick"}));
            done();
          });
        });
    });
    it("should respond with 200, and overwrite the resource, on PUT where resource exists", function(done) {
      chai.request("http://localhost:8080")
        .put("/api/users/annebuch")
        .send(anneData)
        .end(function(err, res) {
          expect(err).to.eql(null);
          expect(res).to.have.status(200);
          fs.readFile("./data/annebuch.json", function(err, data) {
            expect(data.toString()).to.eql(JSON.stringify(anneData));
            done();
          });
        });
    });
  ...
  describe("PATCH request", function() {
    ...
    it("should respond with 200, and update the resource, on PATCH where resource exists", function(done) {
      chai.request("http://localhost:8080")
        .patch("/api/users/annebuch")
        .send({"emailconfirmed":true})
        .end(function(err, res) {
          expect(err).to.eql(null);
          expect(res).to.have.status(200);
          fs.readFile("./data/annebuch.json", function(err, data) {
            expect(data.toString()).to.eql(JSON.stringify(anneUpdated));
            done();
          });
        });
    });
